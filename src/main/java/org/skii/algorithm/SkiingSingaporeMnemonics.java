...
public class SkiingSingaporeMnemonics {
	...
	private static List<Integer> recursion(int x,int i,int j) {
		...
		//if the longest path is already there then no need to go through recursive cycles
			...
			return getLongestPath(listA,listB,listC,listD,x,i,j);
	}
	...
	private static List<Integer> getLongestPath(List<Integer> listA, List<Integer> listB, List<Integer> listC,
			List<Integer> listD,int x,int i,int j) {
			if(listA.size()==0 && listB.size()==0 && listC.size()==0 && listD.size()==0){
				List<Integer> listX=new ArrayList<Integer>();
				listX.add(x);
				mnemonicsMap.put(i+""+j,new ArrayList<Integer>(listX)); 
				return listX;
			}
		...
		mnemonicsMap.put(i+""+j,new ArrayList<Integer>(tempList.get(0)));     
